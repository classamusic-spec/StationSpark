 ...
 */
export const potOrder = ['garlic', 'onion', 'carrot', 'potato', 'tomato', 'chili', 'pepper', 'mushroom', 'lemon'];

/**
 * Ajo borrows the onion's picture and chile borrows the pepper's, so a pot
 * holding both would be asking a child to tell two identical drawings apart.
 * Every list below is filtered through `distinctIcons` before it reaches the
 * counter, which keeps the longer pot order usable without redrawing anything.
 */

/** Food that belongs to another dish — the reason the child has to read, not grab. */
const counterExtras = ['strawberry', 'banana', 'apple', 'grape', 'watermelon', 'corn', 'olive', 'cookie', 'cheese'];
...
export const generateSoupPot: ChallengeGenerator<'soup-pot'> = (ctx) => {
  ...
  /* keep the pot's own order, drop a few steps, then drop any picture twin */
  const keep = rng
    .shuffle(potOrder.map((_, i) => i))
    .slice(0, stepCount + 2)
    .sort((a, b) => a - b);
  const inPot = distinctIcons(
    keep.map((i) => potOrder[i]).filter((id): id is string => id !== undefined).map(wordById),
  ).slice(0, stepCount);
  ...
  const extras = distinctIcons(rng.shuffle(counterExtras).map(wordById), inPot).slice(
    0,
    ageBand === 'A' ? 2 : 3,
  );
